...
public interface TrafficDataRepository extends JpaRepository<TrafficData, Long> {
     ...
     */
    Optional<TrafficData> findTopByLocationOrderByMeasuredAtDesc(Location location);
     ...
     * @return 기간 내 교통 데이터
     */
    List<TrafficData> findByLocationAndMeasuredAtBetweenOrderByMeasuredAtDesc(
     ...
     * @return 페이징된 교통 데이터
     */
    Page<TrafficData> findByLocationOrderByMeasuredAtDesc(Location location, Pageable pageable);
     ...
     */
    List<TrafficData> findByCongestionLevelInOrderByMeasuredAtDesc(
    ...
    @Query("SELECT td.hourOfDay, AVG(td.vehicleCount), AVG(td.averageSpped) " +
            "FROM TrafficData td" +
            "WHERE td.location = :location " +
            "AND td.measuredAt BETWEEN :startDate AND :endDate" +
     ...
     */
    @Query("SELECT td.dayOfWeek, AVG(td.vehicleCount), AVG(td.averageSpped) " +
            "FROM TrafficData td" +
            "WHERE td.location = :location " +
            "AND td.measuredAt BETWEEN :startDate AND :endDate" +
            "GROUP BY td.dayOfWeek" + "ORDER BY td.dayOfWeek")
     ...
     */
    @Query("SELECT td.weatherCondition, AVG(td.vehicleCount), AVG(td.averageSpped)" +
            "FROM TrafficData td" +
            "WHERE td.location = :location" +
